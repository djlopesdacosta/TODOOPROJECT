...
def Backup():
    ...
    exclude_dirs =  ['plugin.program.jogosEmuladores','repository.tdbegley.official','backupdir','cache', 'system', 'Thumbnails']
...
def ARCHIVE_CB(sourcefile, destfile, message_header, message1, message2, message3, exclude_dirs, exclude_files):
    ...
    for base, dirs, files in os.walk(sourcefile):
        ...
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.jogosEmuladores' in dirs:
                   import time
                   FORCE= '01/01/1980'
